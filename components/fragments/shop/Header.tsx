...
const ShopHeader = ({ data }: ShopHeaderProps) => {
  const router = useRouter();

  return (<Card className="flex flex-col space-y-8 lg:flex-row lg:space-y-0 w-full items-center lg:justify-between py-4 px-4 lg:py-6 lg:px-8 border-none shadow-md">
    <div className="flex flex-col sm:flex-row sm:space-x-8 space-y-4 sm:space-y-0 w-full sm:w-auto">
      ...
      <div className="flex flex-col items-center sm:items-start">
        ...
        >
          <Image className="w-5 h-5 sm:w-6 sm:h-6 mr-1" src={Chat} alt="chat" />
          <h3>Chat Penyewa</h3>
        </Button>
      </div>
    </div>
