...
const InputBiodataContent = () => {
  ...
  const [isHandphoneDisabled, setIsHandphoneDisabled] = useState(false)

  useEffect(() => {

    const registerBy = localStorage.getItem("register_by")
    setIsEmailDisabled(registerBy === "email")
    setIsHandphoneDisabled(registerBy === "phone_number")
    ...
    if (storedPassword) {
      try {
        const decryptedPass = CryptoJS.AES.decrypt(storedPassword, SECRET_KEY).toString(CryptoJS.enc.Utf8)
  ...
  const validateForm = () => {
    const newErrors = {
      ...
      fullname: !fullname.trim() ? "Nama lengkap tidak boleh kosong" : fullname.trim().length < 3 ? "Nama lengkap minimal 3 karakter" : "",
  ...
  const handleImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 1048576) {
      setAlertState({ isOpen: true, message: "Ukuran gambar maksimal 1MB" })
      return
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!profileImage.trim()) {
      setAlertState({ isOpen: true, message: "Foto Profile Tidak Boleh Kosong!" })
      return
    }
      ...
      {isModalOpen && (
        <div className='fixed mt-0 inset-0 bg-black bg-opacity-75 flex justify-center items-center z-50 '>
          <div className='relative flex justify-center items-center'>
            ...
            <Image
              // [FIX] Logika fallback gambar diperbaiki
      ...
      <div className='flex flex-col lg:flex-row-reverse w-full space-y-5'>
        <div className='flex flex-col items-center w-full space-y-6 mt-5'>
          <Image
            ...
            width={200}
            height={200}
            // [FIX] Hanya bisa diklik jika ada gambar
            onClick={() => profileImage && setIsModalOpen(true)}
          />
