...
const MetodePembayaranLayout = ({ checkoutDetail }: { checkoutDetail?: TransactionResponseProps }) => {
  ...
  const handlePayment = () => {
    ...
    localStorage.setItem("grandTotalPayment", checkoutDetail?.grand_total_payment?.toString() ?? "0")
  ...
  return (
    <Card className='px-2 md:px-6 mb-[224px]'>
      <CardHeader className='flex flex-col items-center md:flex-row md:justify-between px-0'>
        <h2 className='text-md font-semibold text-color-primary'>Metode Pembayaran</h2>
        ...
        <div className='flex-col max-w-[500px] w-full space-y-[14px]'>
          <div className='flex justify-between'>
            <div className='flex space-x-2'>
              <h3 className='text-color-primary text-[12px] md:text-sm max-w-[110px] sm:max-w-full'>Subtotal untuk Produk</h3>
            </div>
            <h3 className='text-color-primary text-[12px] md:text-sm font-semibold'>{formatToRupiah(checkoutDetail?.sub_total_product_price) || "gratis"}</h3>
          </div>

          <div className='flex justify-between'>
            <div className='flex space-x-2'>
              <h3 className='text-color-primary text-[12px] md:text-sm max-w-[110px] sm:max-w-full'>Subtotal Ongkos Kirim </h3>
            </div>
            <h3 className='text-color-primary text-[12px] md:text-sm font-semibold'>{formatToRupiah(checkoutDetail?.sub_total_shipping_cost) || "gratis"}</h3>
          </div>
          <div className='flex justify-between'>
            <h3 className='text-color-primary text-[12px] md:text-sm'>Subtotal Deposit</h3>

            <h3 className='text-color-primary text-[12px] md:text-sm font-semibold'>{formatToRupiah(checkoutDetail?.sub_total_deposit) || "gratis"}</h3>
          </div>
          <div className='flex justify-between'>
            <h3 className='text-color-primary text-[12px] md:text-sm'>Biaya Layanan</h3>

            <h3 className='text-color-primary text-[12px] md:text-sm font-semibold'>{formatToRupiah(checkoutDetail?.service_fee) || "gratis"}</h3>
          </div>

          <div className='flex justify-between pb-10'>
            <h3 className='text-color-primary text-[12px] md:text-sm max-w-[110px] sm:max-w-full'>Total Pembayaran</h3>

            <h3 className='text-color-secondary text-[14px] lg:text-lg font-bold'>{formatToRupiah(checkoutDetail?.grand_total_payment) || "gratis"}</h3>
          </div>
          <Button
